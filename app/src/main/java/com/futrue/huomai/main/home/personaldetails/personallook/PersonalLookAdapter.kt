...
class PersonalLookAdapter @Inject constructor() :
    BaseQuickAdapter<IBean, BaseViewHolder>(R.layout.item_personallook) {
    override fun convert(helper: BaseViewHolder, item: IBean) {
        helper.apply {
            setText(R.id.tv_year, "2019")
            setText(R.id.tv_day, "19")
            setText(R.id.tv_month, "/11")
            setText(
                R.id.tv_content, "/小罐茶功夫套装骨瓷茶具套组,体验升乐茶香。综合借鉴传统茶具，造型" +
                        "精髓，在此基础上加以改造，使茶具更简单，间接优雅，品味非凡。厚薄均匀的哑光蔓延茶具，彰显..."
            )
            setGone(R.id.ll_1, true)
            setGone(R.id.iv_1, true)
            setGone(R.id.iv_2, true)
            GlideUtil.loadRoundCornersImg(mContext, "", getView(R.id.iv_1))
            GlideUtil.loadRoundCornersImg(mContext, "", getView(R.id.iv_2))
            ...
            setText(R.id.bt_locationLabel, "重庆解放碑")
